...
def parse_data(example_serialized, has_3d=False):
    """Parses tf file.
    It's contents are:
        ...
        'image/ll_idx'       : _int64_feature(index)
    ...
    ll_idx = tf.cast(features['image/ll_idx'], dtype=tf.int32)
    filename = features['image/filename']

    return image, image_size, ll_idx, filename

def parse_example_proto(example_serialized, has_3d=False):
    """HMR code, could be adopted if know center & scale of person
    It's contents are:
...
def jitter_center(center, trans_max):
    with tf.name_scope(None, 'jitter_center', [center, trans_max]):
        rand_trans = tf.random_uniform(
            [2], minval=-trans_max, maxval=trans_max, dtype=tf.int32)
        return center + rand_trans

def jitter_scale_no_jt(image, image_size, center, scale_range):
    with tf.name_scope(None, 'jitter_scale', [image, image_size]):
        ...
        return new_image, tf.cast(tf.stack([cx, cy]), tf.int32)
...
def pad_image_edge(image, margin, display=False):
    ...
    with tf.name_scope(None, 'pad_image_edge', [image, margin]):
        if display:
            top = tf.ones_like(repeat_col(image[0, :, :], margin))
            bottom = tf.ones_like(repeat_col(image[-1, :, :], margin))
        else:
        ...
        if display:
            left = tf.transpose(tf.ones_like(repeat_col(image[:, 0, :], margin)), perm=[1, 0, 2])
            right = tf.transpose(
                tf.ones_like(repeat_col(image[:, -1, :], margin)), perm=[1, 0, 2])
        else:
