...
def preprocess_generic(new_img, res, display=False):
    """pad / resize generic image."""
    border_color = (0, 0, 0)
    if display:
        border_color = (255,255,255)

    if np.shape(new_img)[1] > np.shape(new_img)[0]:
        pad_size = int(.5 * (np.shape(new_img)[1] - np.shape(new_img)[0]))
        new_img = cv2.copyMakeBorder(new_img, top=pad_size, bottom=pad_size,
                    left=0, right=0, 
                    borderType=cv2.BORDER_CONSTANT, value=border_color)
    elif np.shape(new_img)[1] < np.shape(new_img)[0]:
        pad_size = int(.5 * (np.shape(new_img)[0] - np.shape(new_img)[1]))
        new_img = cv2.copyMakeBorder(new_img, top=0, bottom=0,
                    left=pad_size, right=pad_size, 
                    borderType=cv2.BORDER_CONSTANT, value=border_color)

    out_img = cv2.resize(new_img, dsize=(res,res))
